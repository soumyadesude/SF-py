...
def str_function_cpu(Vx, Vy, Vz, Ix, Iy, Iz, l_cap_x, l_cap_y, l_cap_z, S_array_cpu, S_u_r_array_cpu, S_ux_array_cpu, S_uy_array_cpu, S_uz_array_cpu):
    ...
    for m in range(N):
        ...
        S_upll_array_cpu[Ix[m], Iy[m], Iz[m]] = np.mean(diff_magnitude_sqr[:, :, :]*(del_u[:, :, :]*l_cap_x[:, :, :] + del_v[:, :, :]*l_cap_y[:, :, :] + del_w[:, :, :]*l_cap_z[:, :, :]))

        S_u_r_array_cpu[Ix[m], Iy[m], Iz[m]] = np.mean((del_u[:, :, :]*l_cap_x[:, :, :] + del_v[:, :, :]*l_cap_y[:, :, :] + del_w[:, :, :]*l_cap_z[:, :, :])**3)

        

        S_ux_array_cpu[Ix[m], Iy[m], Iz[m]] = np.mean(diff_magnitude_sqr[:, :, :]*(del_u[:, :, :]*l_cap_x[:, :, :]))

        S_uy_array_cpu[Ix[m], Iy[m], Iz[m]] = np.mean(diff_magnitude_sqr[:, :, :]*(del_v[:, :, :]*l_cap_y[:, :, :]))

        S_uz_array_cpu[Ix[m], Iy[m], Iz[m]] = np.mean(diff_magnitude_sqr[:, :, :]*(del_w[:, :, :]*l_cap_z[:, :, :]))
...
def str_function_gpu(Vx, Vy, Vz, Ix, Iy, Iz, l_cap_x, l_cap_y, l_cap_z, S_upll_array, S_u_r_array, S_ux_array, S_uy_array, S_uz_array):
    ...
    for m in range(N):
        ...
        S_upll_array[Ix[m], Iy[m], Iz[m]] = cp.mean(diff_magnitude_sqr[:, :, :]*(del_u[:, :, :]*l_cap_x[:, :, :] + del_v[:, :, :]*l_cap_y[:, :, :] + del_w[:, :, :]*l_cap_z[:, :, :]))

        S_u_r_array[Ix[m], Iy[m], Iz[m]] = cp.mean((del_u[:, :, :]*l_cap_x[:, :, :] + del_v[:, :, :]*l_cap_y[:, :, :] + del_w[:, :, :]*l_cap_z[:, :, :])**3)

        

        S_ux_array[Ix[m], Iy[m], Iz[m]] = cp.mean(diff_magnitude_sqr[:, :, :]*(del_u[:, :, :]*l_cap_x[:, :, :]))

        S_uy_array[Ix[m], Iy[m], Iz[m]] = np.mean(diff_magnitude_sqr[:, :, :]*(del_v[:, :, :]*l_cap_y[:, :, :]))

        S_uz_array[Ix[m], Iy[m], Iz[m]] = cp.mean(diff_magnitude_sqr[:, :, :]*(del_w[:, :, :]*l_cap_z[:, :, :]))
